...
fn is_supported_extension(ext: &str) -> bool {
    matches!(
        ...
        "pdf" | "txt" | "md" | "doc" | "docx" | "odt" | "xls" | "xlsx" | "csv" | "ppt" | "pptx" | "jpg" | "jpeg" | "png"
    )
}
...
fn normalize_filename(name: &str) -> (String, Vec<String>) {
    ...
    let bad_terms = [
        "final_final", "versao boa", "versão boa", "copia", "cópia", "copy",
        "download", "novo", "sem titulo", "sem título", "arquivo", "documento",
        "(1)", "(2)", "(3)", "()"
    ];
...
fn format_date_prefix(modified_at: Option<DateTime<Utc>>) -> (String, String) {
    if let Some(date) = modified_at {
        (date.format("%Y-%m-%d").to_string(), "date_from_modified_time".to_string())
    } else {
        (Utc::now().format("%Y-%m-%d").to_string(), "date_from_current_time".to_string())
    }
}
...
pub fn suggest_rename(file: &FileMetadata) -> Option<RenameSuggestion> {
    ...
    let base_name = if !ext.is_empty() {
        ...
        file.filename.strip_suffix(&suffix).unwrap_or(&file.filename).to_string()
    } else {
        file.filename.clone()
    };
    ...
    if title.to_lowercase().ends_with(&format!("_{}", version.to_lowercase())) {
    ...
    let is_office = matches!(ext.as_str(), "pdf" | "doc" | "docx" | "odt" | "xls" | "xlsx" | "ppt" | "pptx");
    ...
    if title == "Documento_Revisar" { confidence -= 0.3; }
    if ext.is_empty() { confidence -= 0.2; }
...
mod tests {
    ...
    fn test_full_suggestion() {
        ...
        assert_eq!(suggestion.suggested_filename, "2026-05-09_Trabalho_Final_Joao_v1.pdf");
    ...
    fn test_preserve_extension() {
        ...
        let suggestion = suggest_rename(&file).unwrap();
        assert_eq!(suggestion.suggested_filename, "2026-05-09_Comprovante_v1.txt");
    ...
    fn test_already_clean_name() {
        ...
        if let Some(s) = suggestion {
            assert_eq!(s.suggested_filename, "2026-05-09_2026_05_09_Relatorio_Mensal_v1.txt");
